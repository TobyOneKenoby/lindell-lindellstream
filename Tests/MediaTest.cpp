...
int main(int argc,char** argv){try{
 if(argc==4&&std::string(argv[1])=="--browser"){
  ...
  if(std::getenv("LSL_TEST_TLS_RELAY")){config.iceServers.emplace_back("turns:localhost:18444?transport=tcp");config.iceServers.back().username="fixture";config.iceServers.back().password="fixture-password";config.iceTransportPolicy=rtc::TransportPolicy::Relay;}
  ...
  while((answer=peer->answer()).empty()&&std::chrono::steady_clock::now()<deadline)std::this_thread::sleep_for(std::chrono::milliseconds(10));check(!answer.empty(),"ICE gathering timed out");std::ofstream(argv[3])<<answer;
  deadline=std::chrono::steady_clock::now()+std::chrono::seconds(15);while(!peer->connected()&&std::chrono::steady_clock::now()<deadline)std::this_thread::sleep_for(std::chrono::milliseconds(10));check(peer->connected(),"Browser did not connect");
