...
int main(int argc,char** argv){try{
 if(argc==4&&std::string(argv[1])=="--browser"){
  std::ifstream input(argv[2]);std::string offer((std::istreambuf_iterator<char>(input)),{});rtc::Configuration config;config.disableAutoNegotiation=true;auto peer=std::make_shared<lsl::MediaPeer>(config,offer,"test");
