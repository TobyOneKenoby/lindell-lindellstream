...
(async()=>{
 ...
 try{
 await new Promise(r=>server.listen(0,'127.0.0.1',r));browser=await chromium.launch({headless:true,args:['--autoplay-policy=no-user-gesture-required']});const page=await browser.newPage();await page.goto(`http://127.0.0.1:${server.address().port}`);
 const offer=await page.evaluate(async()=>{
  ...
  pc.ontrack=e=>{const source=audio.createMediaStreamSource(new MediaStream([e.track]));const splitter=audio.createChannelSplitter(2);source.connect(splitter);for(let c=0;c<2;c++){const a=audio.createAnalyser();a.fftSize=4096;splitter.connect(a,c);window.analysers.push(a);}const mute=audio.createGain();mute.gain.value=0;source.connect(mute).connect(audio.destination);};
 ...
 const results=await page.evaluate(async()=>{
  ...
  const channels=analysers.map(measure);let bytes=0;for(const stat of (await pc.getStats()).values())if(stat.type==='inbound-rtp'&&stat.kind==='audio')bytes+=stat.bytesReceived;return {channels,bytes};
 });
